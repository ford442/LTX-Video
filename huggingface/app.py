...
def compile_transformer():
    with spaces.aoti_capture(pipeline_instance.transformer) as call:
        dummy_image = Image.new("RGB", (1024, 1024))
        timesteps = [1.0000, 0.9937, 0.9875, 0.9812, 0.9750, 0.9094, 0.7250]
        pipeline_instance(prompt="a robot dancing in the rain",
        image=dummy_image,
        num_frames=60,
        width=1024,
        height=1024,
...
#spaces.aoti_apply(compiled_transformer, pipeline_instance.transformer)
...
def generate_image(prompt, negative_prompt,
    ...
    try:
        ...
        if enable_teacache:
            print(f"✅ T2I TeaCache is ENABLED with threshold: {teacache_threshold}")
    ...
    call_kwargs = {
        ...
        "num_frames": num_frames_padded, "num_inference_steps": num_steps, "frame_rate": 1, # Frame rate is irrelevant
    ...
    stg_map = {"stg_av": SkipLayerStrategy.AttentionValues, "attention_values": SkipLayerStrategy.AttentionValues}
    call_kwargs["skip_layer_strategy"] = stg_map.get(stg_mode_str, SkipLayerStrategy.AttentionValues)

    # --- 4. Configure TeaCache for single pass ---
    ...
    # --- 6. Extract First Frame & Latent ---
    # Get the *first* frame, keeping the 'N' dimension (shape [b, c, 1, h, w])
    first_frame_unnormalized = result_images_tensor[:, :, [0], :, :].clone().cpu()
    
    # Normalize it to -1 to 1 range for the latent state
    latent_to_return = (first_frame_unnormalized * 2.0) - 1.0
    output_tensor_toggle = gr.update(visible=True, value=True) # Show and check the tensor toggle

    # Create PIL image for display (0-1 range)
    # Squeeze from [1, 3, 1, h, w] down to [3, h, w] by removing all unit dims
    pil_image_tensor = first_frame_unnormalized.squeeze() 
    
    # Permute from [c, h, w] to [h, w, c] for PIL
    pil_image_np = (pil_image_tensor.permute(1, 2, 0).cpu().float().numpy() * 255).astype(np.uint8)
    pil_image = Image.fromarray(pil_image_np)
    display_image = pil_image.copy()
    
    i2v_image_path = tempfile.mktemp(suffix=".png")
    display_image.save(i2v_image_path)
...
def generate(prompt, negative_prompt, clips_list, input_image_filepath, input_video_filepath, 
             last_frame_tensor_from_state, # <-- ADDED
             height_ui, width_ui, mode, duration_ui, ui_frames_to_use, 
             seed_ui, randomize_seed, ui_guidance_scale, improve_texture_flag, 
             use_temporal_upscaler_flag, use_last_tensor_flag, # <-- ADDED
             num_steps, fps,
    ...
    if use_last_tensor_flag and last_frame_tensor_from_state is not None:
        ...
        media_tensor = last_frame_tensor_from_state.to(target_inference_device) # Shape [b, c, 1, h_old, w_old]
        b, c, n, h, w = media_tensor.shape
        
        # Flatten batch and frame dims for 4D resize, as interpolate expects 4D
        ...
        # Un-flatten back to 5D
        media_tensor_5d = resized_tensor_4d.view(b, c, n, actual_height, actual_width)
        
        # Now pad and set as conditioning item
        call_kwargs["conditioning_items"] = [ConditioningItem(torch.nn.functional.pad(media_tensor_5d, padding_values).to(target_inference_device), 0, 1.0)]
        print(f"Set tensor input with new shape: {media_tensor_5d.shape}")
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Row():
        with gr.Column():
            with gr.Tabs() as tabs:
                with gr.Tab("Text-to-Image (Primer)", id="t2i_tab"):
                    ...
                    t2i_button = gr.Button("Generate Image & Prime Latent", variant="primary")
    ...
    common_params = [height_input, width_input, mode, duration_input, frames_to_use, seed_input, randomize_seed_input, guidance_scale_input, improve_texture, use_temporal_upscaler, use_last_tensor_toggle, num_steps, fps, teacache_checkbox, teacache_slider]
